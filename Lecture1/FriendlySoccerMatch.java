...
public class FriendlySoccerMatch implements FriendlyMatch {
	...
	float strength_1 = (t1.getStrength()/2.0f) +
	((t1.getStrength()/2.0f)*(t1.getMotivation()/10.0f));
	float strength_2 = (t2.getStrength()/2.0f) +
	((t2.getStrength()/2.0f) * (t2.getMotivation()/10.0f));
	...
	if ((r.nextInt(Math.round(strength_1+strength_2))-
	strength_1)<=0){
	...
	if (goal) {
		pointsHome++;
		p.addGoal();
		String[] textGoal = {"AMAZING, THEY ACTUALLY SCORED!","IS THIS EVEN POSSIBLE?!", "GIVE THAT MAN A DOLLAA", "HE WOULD DO IT AGAIN IF HE COULD, ha"};
		Random random1 = new Random();
		int select = random1.nextInt(textGoal.length);
		System.out.println(textGoal[select]+pointsHome+":"+
	...
	if (goal) {
		pointsGuest++;
		p.addGoal();
		String[] textGoal = {"BEST SHOT EVAR" ,"FANTASTIC SHOT", "OH MY GOD, HE SCORES", "GOLGOLGOLGOLGOLGOLGOLGOLGOOOOAAAAALLLL"};
		Random random1 = new Random();
		int select = random1.nextInt(textGoal.length);
		System.out.println(textGoal[select]+pointsHome+":"+
		pointsGuest+" "+p.getName()+"("+p.getGoals()+")");
